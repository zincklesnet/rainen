...
if ( 0 == PeepSo::get_option( 'disable_navbar', 0 ) ) {
	...
	<?php if ( is_user_logged_in() ) { ?>
		...
		<div class="pso-navbar <?php echo esc_attr( $navbar_sticky ); ?> js-toolbar">
			...
			<div class="pso-navbar__tabs"><?php echo wp_kses_post( $peepso_general->render_navigation( 'primary' ) ); ?></div>
			<div class="pso-navbar__tabs pso-navbar__tabs--mobile"><?php echo wp_kses_post( $peepso_general->render_navigation( 'mobile-secondary' ) ); ?></div>
			<div class="pso-navbar__user">
				<div class="pso-dropdown pso-navbar-user__dropdown ps-js-dropdown">
					...
					<div class="pso-dropdown__menu ps-js-dropdown-menu">
						<?php echo wp_kses_post( $peepso_general->render_navigation( 'user' ) ); ?>
					</div>
				</div>
				<div class="pso-navbar__notifs"><?php echo wp_kses_post( $peepso_general->render_navigation( 'secondary' ) ); ?></div>
			</div>
			...
			<?php echo wp_kses_post( $peepso_general->render_navigation( 'mobile-primary' ) ); ?>
			</div>
		</div>
		<!-- end: PeepSo Navbar -->
	<?php }
