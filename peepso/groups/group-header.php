...
$peepso_group_user = new PeepSoGroupUser( $group->id );
$peepso_group     = $group;
$cover_url        = $peepso_group->get_cover_url();
$has_cover       = false;
...
if ( false !== stripos( $cover_url, 'peepso/groups/' ) ) {
	$has_cover = true;
}

if ( false === $peepso_group_user->can( 'manage_group' ) || ( false === $has_cover ) ) {
...
<div class="ps-focus ps-focus--group ps-group__profile-focus ps-js-focus ps-js-focus--group ps-js-group-header <?php echo esc_attr( $group_avatar_class ); ?>">
	<div class="ps-focus__cover ps-js-cover">
		<div class="ps-focus__cover-image ps-js-cover-wrapper">
			...
			$cover_alt = sprintf( esc_attr__( '%s cover photo', 'reign' ), esc_attr( $peepso_group->get( 'name' ) ) );
			?>
			<img class="ps-js-cover-image" src="<?php echo esc_url( $group_cover_photo ); ?>"
				alt="<?php echo esc_attr( $cover_alt ); ?>"
				style="<?php echo esc_attr( $peepso_group->cover_photo_position() ); ?>; opacity: 0;" />
		...
		<?php
		$cover_box_attrs = '';
		if ( $peepso_group->has_cover() ) {
			$cover_box_attrs = ' style="cursor:pointer" data-cover-url="' . esc_url( $peepso_group->get_cover_url() ) . '"';
		}
		?>

		<div class="ps-focus__cover-inner ps-js-cover-button-popup"<?php echo $cover_box_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<div class="ps-avatar ps-avatar--focus ps-focus__avatar ps-group__profile-focus-avatar ps-js-avatar">
				<?php
				/* translators: %s: Group name. */
				$avatar_alt = sprintf( esc_attr__( '%s avatar', 'reign' ), esc_attr( $peepso_group->get( 'name' ) ) );
				?>
				<img class="ps-js-avatar-image" src="<?php echo esc_url( $peepso_group->get_avatar_url_full() ); ?>"
					alt="<?php echo esc_attr( $avatar_alt ); ?>" />

				<?php
				$avatar_box_attrs = ' style="cursor:default"';
				if ( $peepso_group->has_avatar() ) {
					$avatar_box_attrs = ' onclick="peepso.simple_lightbox(\'' . esc_url( $peepso_group->get_avatar_url_orig() ) . '\'); return false"';
				}
				?>

				<div class="ps-focus__avatar-change-wrapper ps-js-avatar-button-wrapper"<?php echo $avatar_box_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
					<?php if ( $peepso_group_user->can( 'manage_group' ) ) { ?>
	...
	<div class="ps-focus__footer ps-group__profile-focus-footer">
		<div class="ps-focus__info">
			...
			<div class="ps-focus__desc ps-js-focus-desc">
				...
				<div class="ps-focus__desc-details">
					...
					foreach ( $group_categories as $peepso_group_category ) {
						echo '<a href="' . esc_url( $peepso_group_category->get_url() ) . '">' . esc_html( $peepso_group_category->name ) . '</a>';
					}
					?>
				</div>
				<?php } ?>
			...
			<div class="ps-focus__details">
				...
				<div class="ps-focus__detail">
					<?php if ( $peepso_group_user->can( 'manage_group' ) && strlen( $group_segment ) && 'settings' == $group_segment ) { ?>
						<div class="ps-group__profile-privacy ps-dropdown ps-dropdown--privacy ps-js-dropdown ps-js-privacy ps-js-privacy--<?php echo esc_attr( $group->id ); ?>">
							...
							<?php echo wp_kses_post( PeepSoGroupPrivacy::render_dropdown() ); ?>
						</div>
					<?php } else { ?>
						<span class="ps-btn ps-btn--sm ps-btn--app ps-tip ps-tip--bottom ps-tip--md ps-tip--arrow ps-tip--left" aria-label="<?php echo esc_attr( $group->privacy['desc'] ); ?>">
							<i class="<?php echo esc_attr( $group->privacy['icon'] ); ?>"></i>
							<?php
							/* translators: %s: Group privacy label (e.g. Public, Private). */
							printf( esc_html__( ' %s Group', 'reign' ), esc_html( $group->privacy['name'] ) );
							?>
						</span>
					<?php } ?>
				</div>
				...
				<a class="ps-focus__detail" href="<?php echo esc_url( $group->get_url() . 'members/' ); ?>">
					...
					<span class="ps-js-member-count">
						...
						printf( esc_html( _n( '%s member', '%s members', $group->members_count, 'reign' ) ), esc_html( number_format_i18n( $group->members_count ) ) );
						?>
					</span>
				</a>

				<!-- Pending members -->
				<?php if ( $group->pending_admin_members_count > 0 && $peepso_group_user->can( 'manage_users' ) ) { ?>
					<a class="ps-focus__detail ps-js-pending-label" href="<?php echo esc_url( $group->get_url() . 'members/pending' ); ?>">
						...
						<?php
						printf(
							'<span class="ps-js-pending-count" data-id="%1$d">%2$s</span> %3$s',
							esc_attr( $group->id ),
							esc_html( $group->pending_admin_members_count ),
							esc_html__( 'pending', 'reign' )
						);
						?>
					</a>
				<?php } ?>
		...
		<div class="ps-focus__menu ps-js-focus__menu">
			<div class="ps-focus__menu-inner ps-js-focus__menu-inner">
				...
				if ( $peepso_group_user->can( 'manage_group' ) ) {
					$segments[0][] = array(
						'href'  => 'settings',
				...
				if ( $peepso_group_user->can( 'manage_users' ) && $pending ) {
						$members_title .= ' <span class="ps-js-pending-label">(' . sprintf(
							/* translators: 1: Group ID, 2: Number of pending members. */
							__( '<span class="ps-js-pending-count" data-id="%1$d">%2$s</span> pending', 'reign' ),
							absint( $group->id ),
							absint( $pending )
						) . ')</span>';
				}

				if ( $peepso_group_user->can( 'view_users' ) ) {
					$segments[0][] = array(
						'href'  => 'members',
						'title' => $members_title,
				...
				$segments['_PeepSoGroup']     = $peepso_group;
				$segments['_PeepSoGroupUser'] = $peepso_group_user;
				...
				foreach ( $segments as $segment_group ) {
					foreach ( $segment_group as $segment ) {
						...
						$can_access = $peepso_group_user->can( 'access_segment', $segment['href'] );
...
<script>
jQuery(function() {
	peepsogroupsdata.group_id = +'<?php echo absint( $group->id ); ?>';
});
</script>
