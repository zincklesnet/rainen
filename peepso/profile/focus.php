...
?>
<div class="ps-focus <?php echo esc_attr( $use_small_cover ? 'ps-focus--small' : '' ); ?> ps-js-focus ps-js-focus--profile ps-js-focus--<?php echo esc_attr( $peepso_user->get_id() ); ?> <?php echo esc_attr( $profile_avatar_class ); ?>">
	<div class="ps-focus__cover ps-js-cover">
		<div class="ps-focus__cover-image ps-js-cover-wrapper">
			<?php
			/* translators: %s: User full name. */
			$cover_alt = sprintf( esc_attr__( '%s cover photo', 'reign' ), esc_attr( $peepso_user->get_fullname() ) );
			?>
			<img class="ps-js-cover-image" src="<?php echo esc_url( $cover ); ?>"
				alt="<?php echo esc_attr( $cover_alt ); ?>"
				style="<?php echo esc_attr( $peepso_user->get_cover_position() ); ?>; opacity: 0;" />
		...
		<?php
		$cover_box_attrs = '';
		if ( $peepso_user->has_cover() ) {
			$cover_box_attrs = ' style="cursor:pointer" data-cover-url="' . esc_url( $peepso_user->get_cover() ) . '"';
		}
		?>

		<?php if ( $peepso_profile->can_edit() ) { ?>
		...
		<div class="ps-focus__cover-inner ps-js-cover-button-popup"<?php echo $cover_box_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<div class="ps-avatar ps-avatar--focus ps-focus__avatar ps-js-avatar">
				<?php
				/* translators: %s: User full name. */
				$avatar_alt = sprintf( esc_attr__( '%s avatar', 'reign' ), esc_attr( $peepso_user->get_fullname() ) );
				?>
				<img class="ps-js-avatar-image" src="<?php echo esc_url( $peepso_user->get_avatar( 'full' ) ); ?>"
					alt="<?php echo esc_attr( $avatar_alt ); ?>" />
				<?php
				$avatar_box_attrs = ' style="cursor:default"';
				if ( $peepso_user->has_avatar() ) {
					$avatar_box_attrs = ' onclick="peepso.simple_lightbox(\'' . esc_js( $peepso_user->get_avatar( 'orig' ) ) . '\'); return false"';
				}
				?>
				...
				<div class="ps-focus__avatar-change-wrapper ps-js-avatar-button-wrapper"<?php echo $avatar_box_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
					<?php if ( ( 1 != PeepSo::get_option( 'avatars_wordpress_only', 0 ) ) && $peepso_profile->can_edit() ) { ?>
				...
				<?php if ( $peepso_user->get_online_status() ) { ?>
					<?php
					/* translators: %s: User full name. */
					$online_label = sprintf( esc_attr__( '%s is currently online', 'reign' ), esc_attr( $peepso_user->get_fullname() ) );
					?>
			...
			<div class="ps-focus__cover-actions ps-js-focus-actions">
				<?php $peepso_profile->profile_actions(); ?>
			</div>
			<div class="reign-social-icons">
				<?php reign_peepso_user_social_links( $peepso_user->get_id() ); ?>
			</div>
		</div>
	...
	<div class="ps-focus__footer">
		<div class="ps-focus__info">
			<div class="ps-focus__title">
				...
				if ( ! $is_profile_segment || 1 == (int) PeepSo::get_option( 'always_full_cover', 0 ) ) {
					...
					do_action( 'peepso_profile_cover_full_before_name', $peepso_user->get_id() );
					echo '</div>';
				}
				?>
				<div class="ps-focus__name" data-hover-card="<?php echo esc_attr( $peepso_user->get_id() ); ?>">
				<?php
					do_action( 'peepso_action_render_user_name_before', $peepso_user->get_id() );
					echo esc_html( $peepso_user->get_fullname() );
					do_action( 'peepso_action_render_user_name_after', $peepso_user->get_id() );
				?>
				</div>
				<?php
				if ( ! $is_profile_segment || 1 == (int) PeepSo::get_option( 'always_full_cover', 0 ) ) {
					echo '<div class="ps-focus__title-after">';
					do_action( 'peepso_profile_cover_full_after_name', $peepso_user->get_id() );
					echo '</div>';
				}
				?>
			</div>
			<div class="ps-focus__details ps-js-focus-interactions">
				<?php $peepso_profile->interactions(); ?>
			</div>
			<div class="ps-focus__mobile-actions ps-js-focus-actions"><?php $peepso_profile->profile_actions(); ?></div>
			<div class="ps-focus__actions ps-js-profile-actions-extra"><?php $peepso_profile->profile_actions_extra(); ?></div>
		</div>

		<?php
		do_action( 'peepso_action_render_user_menu_before', $peepso_user->get_id() );

		if ( PeepSo::is_admin() ) {

			$reports = ( new PeepSoReport() )->get_reports( $peepso_user->get_id(), 0, 0 );
			if ( is_array( $reports ) && count( $reports ) ) {
				$reported = count( $reports );
				update_user_meta( $peepso_user->get_id(), 'peepso_reported', 1 );
				PeepSoTemplate::exec_template(
					'activity',
					'post-reports',
					array(
						'post_id'     => $peepso_user->get_id(),
						'module_id'   => 0,
						'type'        => 'profile',
						'reported'    => $reports,
						'reports'     => $reports,
		...
		<div class="ps-focus__menu ps-js-focus__menu">
			<div class="ps-focus__menu-inner ps-js-focus__menu-inner">
				<?php echo wp_kses_post( $peepso_profile->profile_navigation( array( 'current' => $current ) ) ); ?>
			</div>
