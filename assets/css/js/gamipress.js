jQuery( document ).ready(function( $ ) {
        ...
        $( '.buddypress.widget .gamipress-buddypress-user-details-listing:not(.is_loaded)' ).each ( function() {
            if( $( this ).text().trim() !== '' ) {
                ...
                if( $( this ).find( 'img' ).length ) {
                    ...
                    $( this ).parent().find( '.showGamipressData' ).append( '<i class="far fa-award"></i>');
                }
                $( this ).parent().find( '.gamipress-buddypress-user-details-listing' ).wrap( '<div class="GamiPress-data-popup"></div>' );
                $( this ).parent().find( '.gamipress-buddypress-user-details-listing' ).append( '<i class="far fa-times hideGamipressData"></i>' );
            }
	...
	if( $( '.buddypress.widget .gamipress-buddypress-user-details-listing' ).length ) {
		...
		$( document ).on('click', '.buddypress.widget .showGamipressData', function() {
			...
			if( $( this ).closest( '.widget-area-inner' ).length ) { //Check if parent is sticky
				tempStyles = $( this ).closest( '.widget-area-inner' ).attr( 'style' ); //Store parent's fixed styling and remove to avoid issue
				$( this ).closest( '.widget-area-inner' ).attr( 'style', '' );
		...
		$( document ).on('click', '.buddypress.widget .GamiPress-data-popup .hideGamipressData', function() {
			...
			if( $( this ).closest( '.widget-area-inner' ).length ) {
				$( this ).closest( '.widget-area-inner' ).attr( 'style', tempStyles ); //add parent's fixed styling back
				tempStyles = '';
