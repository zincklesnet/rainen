	...
	class Wbcom_Admin_Settings {
		...
		public function wbcom_enqueue_admin_scripts() {
			...
			if ( ! wp_script_is( 'wbcom_admin_setting_js', 'enqueued' ) ) {
				...
				wp_register_script(
					...
					BUDDYPRESS_SCHEDULE_ACTIVITY_URL . 'admin/wbcom/assets/js' . $path . '/wbcom-admin-setting' . $extension,
					array( 'jquery' ),
					time(),
					false
				);
				wp_localize_script(
					...
					array(
						...
						'activate_text'   => esc_html__( 'Activate', 'buddypress-schedule-activity' ),
						'deactivate_text' => esc_html__( 'Deactivate', 'buddypress-schedule-activity' ),
			...
			if ( ! wp_style_is( 'wbcom-admin-setting-css', 'enqueued' ) ) {
				...
				wp_enqueue_style( 'wbcom-admin-setting-css', BUDDYPRESS_SCHEDULE_ACTIVITY_URL . 'admin/wbcom/assets/css' . $path . '/wbcom-admin-setting' . $extension );
			}
		}
		...
		public function wbcom_admin_additional_pages() {
			if ( empty( $GLOBALS['admin_page_hooks']['wbcomplugins'] ) ) {
				add_menu_page( esc_html__( 'WB Plugins', 'buddypress-schedule-activity' ), esc_html__( 'WB Plugins', 'buddypress-schedule-activity' ), 'manage_options', 'wbcomplugins', array( $this, 'wbcom_license_submenu_page_callback' ), 'dashicons-lightbulb', 59 );

				// add_submenu_page( 'wbcomplugins', esc_html__( 'General', 'buddypress-schedule-activity' ), esc_html__( 'General', 'buddypress-schedule-activity' ), 'manage_options', 'wbcomplugins' );
			}
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Plugins', 'buddypress-schedule-activity' ),
				esc_html__( 'Our Plugins', 'buddypress-schedule-activity' ),
				'manage_options',
				'wbcom-plugins-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Themes', 'buddypress-schedule-activity' ),
				esc_html__( 'Our Themes', 'buddypress-schedule-activity' ),
				'manage_options',
				'wbcom-themes-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Support', 'buddypress-schedule-activity' ),
				esc_html__( 'Support', 'buddypress-schedule-activity' ),
				'manage_options',
				'wbcom-support-page',
					...
					<ul>
						<li class="wb_admin_nav_item <?php echo esc_attr( $settings_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcomplugins'; ?>" id="wb_admin_nav_trigger_settings">
								...
								<h4><?php esc_html_e( 'Settings', 'buddypress-schedule-activity' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $plugin_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-plugins-page'; ?>" id="wb_admin_nav_trigger_extensions">
								...
								<h4><?php esc_html_e( 'Themes & Extension', 'buddypress-schedule-activity' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $support_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-support-page'; ?>" id="wb_admin_nav_trigger_support">
								...
								<h4><?php esc_html_e( 'Help & Support', 'buddypress-schedule-activity' ); ?></h4>
							</a>
						</li>
