...
function cpfb_add_brackets( $field_value ) {
	...
	if ( strpos( $field_value, '[' ) | strpos( $field_value, ']' ) ) { // If there is a bracket in a field, this block overrides the auto-linking
		...
		while ( strpos( $field_value, ']') ) {
			...
			$field_value =  substr($field_value, 0, $open_delin_pos) . '<a href="' . bp_get_members_directory_permalink() . '/?s=' . substr($field_value, $open_delin_pos+1, $close_delin_pos - $open_delin_pos - 1) . '">' . substr($field_value, $open_delin_pos+1, $close_delin_pos - $open_delin_pos - 1) . '</a>' . substr($field_value, $close_delin_pos+1);
		}
	}
