<div class="wrap">
    ...
    <div class="wbcom-wrap">
        ...
        ?>
        <h4 class="wbcom-plugin-heading"><?php esc_html_e('Paid Addons', 'buddypress-quotes'); ?></h4>
                            ...
                            <div class="wbcom_single_right-wrap">
                                ...
                                <div class="activation_button_wrap">
                                    <a href="<?php echo esc_url($plugin_details['download_url']); ?>" class="wb_btn wb_btn_default" target="_blank" >
                                        <i class="fa fa-eye"></i>
                                        <?php esc_html_e('View', 'buddypress-quotes'); ?>
                                    </a>
                                </div>
                            </div>
                        </div>
        ...
        <h4 class="wbcom-plugin-heading"><?php esc_html_e('Free Addons', 'buddypress-quotes'); ?></h4>
        <div class="reign-demos-wrapper reign-importer-section">
            <div class="reign-demos-inner-wrapper wbcom-plugins-inner-wrapper">
                <?php
                foreach ($free_plugins as $key => $plugin_details) {
                    if ('not_installed' == $plugin_details['status']) {
                        $plugin_btn_text = esc_html__('Install', 'buddypress-quotes');
                        $toggle_class = 'fa fa-toggle-off';
                        $plugin_action = 'install_plugin';
                    } else if ('installed' == $plugin_details['status']) {
                        $plugin_btn_text = esc_html__('Activate', 'buddypress-quotes');
                        $toggle_class = 'fa fa-toggle-off';
                        $plugin_action = 'activate_plugin';
                    } else {
                        $plugin_btn_text = esc_html__('Deactivate', 'buddypress-quotes');
