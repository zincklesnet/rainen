 ...
 * @version 1.6.15
 */
class EDD_BNEWS_Plugin_Updater {
		...
		if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			if ( empty( $version_info->download_link ) ) {
				printf(
					...
					esc_html__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'buddypress-newsfeed' ),
				...
				printf(
					/* translators: %1$s: Version Info ; %2$s: changelog link ; %3$s: New Version Info*/
					esc_html__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'buddypress-newsfeed' ),
					esc_html( $version_info->name ),
					'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
					esc_html( $version_info->new_version ),
					'</a>',
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'buddypress-newsfeed' ), esc_html_e( 'Error', 'buddypress-newsfeed' ), array( 'response' => 403 ) );
		}
