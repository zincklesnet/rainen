	...
	class Wbcom_Admin_Settings {
		...
		public function wbcom_enqueue_admin_scripts() {
			if ( ! wp_style_is( 'font-awesome', 'enqueued' ) ) {
				...
				wp_enqueue_style( 'font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css', array(), BNEWS_PLUGIN_VERSION, 'all' );
			}
			if ( ! wp_script_is( 'wbcom_admin_setting_js', 'enqueued' ) ) {
				...
				wp_register_script(
					...
					$src       = BNEWS_PLUGIN_URL . 'admin/wbcom/assets/js' . $path . '/wbcom-admin-setting' . $extension,
					$deps      = array( 'jquery' ),
					$ver       = BNEWS_PLUGIN_VERSION,  // Use the global plugin version
					$in_footer = true
				);
				wp_localize_script(
					...
					array(
						...
						'activate_text'   => esc_html__( 'Activate', 'buddypress-newsfeed' ),
						'deactivate_text' => esc_html__( 'Deactivate', 'buddypress-newsfeed' ),
			...
			if ( ! wp_style_is( 'wbcom-admin-setting-css', 'enqueued' ) ) {
				...
				wp_enqueue_style( 'wbcom-admin-setting-css', BNEWS_PLUGIN_URL . 'admin/wbcom/assets/css' . $path . '/wbcom-admin-setting' . $extension, array(), BNEWS_PLUGIN_VERSION );
			}
		}
		...
		public function wbcom_admin_additional_pages() {
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Plugins', 'buddypress-newsfeed' ),
				esc_html__( 'Our Plugins', 'buddypress-newsfeed' ),
				'manage_options',
				'wbcom-plugins-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Themes', 'buddypress-newsfeed' ),
				esc_html__( 'Our Themes', 'buddypress-newsfeed' ),
				'manage_options',
				'wbcom-themes-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Support', 'buddypress-newsfeed' ),
				esc_html__( 'Support', 'buddypress-newsfeed' ),
				'manage_options',
				'wbcom-support-page',
					...
					<ul>
						<li class="wb_admin_nav_item <?php echo esc_attr( $settings_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcomplugins'; ?>" id="wb_admin_nav_trigger_settings">
								...
								<h4><?php esc_html_e( 'Settings', 'buddypress-newsfeed' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $plugin_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-plugins-page'; ?>" id="wb_admin_nav_trigger_extensions">
								...
								<h4><?php esc_html_e( 'Themes & Extension', 'buddypress-newsfeed' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $support_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-support-page'; ?>" id="wb_admin_nav_trigger_support">
								...
								<h4><?php esc_html_e( 'Help & Support', 'buddypress-newsfeed' ); ?></h4>
							</a>
						</li>
