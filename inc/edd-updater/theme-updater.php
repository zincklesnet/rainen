...
if ( ! class_exists( 'EDD_Reign_Theme_Updater_Admin' ) ) {
	include __DIR__ . '/theme-updater-admin.php';
}
...
$updater = new EDD_Reign_Theme_Updater_Admin(
	...
	array(
		...
		/* translators: 1: activated sites count, 2: total allowed sites. */
		...
		/* translators: 1: theme name, 2: new version, 3: changelog URL, 4: changelog title, 5: update URL, 6: update link attributes. */
		'update-available'          => __( '<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4$s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'reign' ),
	)
);
