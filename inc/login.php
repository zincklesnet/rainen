...
if ( reign_is_truthy( $rg_custom_login ) ) {
...
if ( ! function_exists( 'custom_login_classes' ) ) {
	...
	function custom_login_classes( $classes ) {
		...
		if ( reign_is_truthy( $rg_custom_login ) && $is_login_page ) {
			if ( reign_is_truthy( $rg_admin_background ) && ! reign_is_truthy( $custom_login_theme_toggle ) ) {
				$classes[] = 'login-split-page rg-login';
			} elseif ( reign_is_truthy( $custom_login_theme_toggle ) ) {
		...
		jQuery( document ).ready( function () {
			...
			var loginLogoImage = function() {
				jQuery('.login.rg-login #login > h1 > a').each(function() {
					...
					$this.append( '<img class="bs-cs-login-logo" src="<?php echo esc_js( $rg_logoimg ); ?>" alt="" aria-hidden="true" />' );
				});
			};
...
if ( ! function_exists( 'login_custom_head' ) ) {
	...
	function login_custom_head() {
		...
		$rg_admin_login_background_switch   = get_theme_mod( 'toggle_custom_background', false );
		...
		if ( reign_is_truthy( $rg_admin_login_background_switch ) && ! reign_is_truthy( $custom_login_theme_toggle ) ) {
			...
			if ( $rg_admin_login_background_text ) {
				echo wp_kses_post( $rg_admin_login_background_text );
			}
			if ( $rg_admin_login_background_textarea ) {
				echo '<span>';
				echo wp_kses_post( stripslashes( $rg_admin_login_background_textarea ) );
				echo '</span>';
			}
		...
		// Logo Variables.
		$rg_logoimg = get_theme_mod( 'admin_logo_media' );
		// The slider control stores these WITH a unit suffix (e.g. '200px'); the
		// CSS below appends 'px' again, so the raw mod produced 'width:200pxpx'
		// (invalid - the browser dropped it and the logo size never applied).
		// Reduce to the bare numeric portion so the single 'px' append below is
		// valid. Handles new '200px' saves, legacy bare '200' saves, and the
		// '145px' default alike.
		$rg_logowidth              = preg_replace( '/[^0-9.\-]/', '', (string) get_theme_mod( 'admin_logo_width' ) );
		$admin_logo_spacing        = preg_replace( '/[^0-9.\-]/', '', (string) get_theme_mod( 'admin_logo_spacing' ) );
		...
		// Values saved by the former sanitize_key() callback had slashes and dots
		// stripped, producing a mangled non-URL string. Detect these by checking
		// for a valid scheme + a non-empty path; fall back to the default gallery
		// image so the CSS url() is always valid even before a re-save heals the DB.
		if ( $login_custom_background_gallery ) {
			$_parsed_gallery = wp_parse_url( $login_custom_background_gallery );
			$_valid_scheme   = isset( $_parsed_gallery['scheme'] ) && in_array( $_parsed_gallery['scheme'], array( 'http', 'https' ), true );
			$_valid_path     = ! empty( $_parsed_gallery['path'] ) && '/' !== $_parsed_gallery['path'];
			if ( ! $_valid_scheme || ! $_valid_path ) {
				$login_custom_background_gallery = REIGN_THEME_URI . '/lib/images/gallery/img-1.jpg';
			}
			unset( $_parsed_gallery, $_valid_scheme, $_valid_path );
		}
		...
		if ( reign_is_truthy( $custom_login_theme_toggle ) && 'modern' === $custom_login_choose_theme ) {
			...
			if ( $login_modern_heading ) {
				echo wp_kses_post( $login_modern_heading );
			}
			if ( $login_modern_text ) {
				echo '<span>';
				echo wp_kses_post( stripslashes( $login_modern_text ) );
				echo '</span>';
			}
			echo '</div><div class="split-overlay"></div></div>';
		}
		...
		if ( reign_is_truthy( $login_background_video_toggle ) && $login_background_video ) {
		...
		if ( reign_is_truthy( $rg_admin_login_background_switch ) && $rg_login_background_media ) {
			?>
			.login-split {
		...
		if ( $rg_admin_login_overlay_opacity ) {
			...
			@media( min-width: 992px ) {
				body.login.login-split-page .login-split .split-overlay {
					opacity: <?php echo esc_attr( floatval( $rg_admin_login_overlay_opacity ) / 100 ); ?>;
				}
			}
			<?php
		}
		...
		if ( reign_is_truthy( $custom_login_theme_toggle ) && $custom_login_choose_theme && $login_custom_background_gallery ) {
			?>
			body.login {
		...
		if ( reign_is_truthy( $custom_login_theme_toggle ) && $custom_login_choose_theme && $login_custom_background_image ) {
			?>
			body.login {
		...
		if ( $login_modern_overlay_opacity ) {
			...
			@media( min-width: 992px ) {
				.login.rg-login.login-modern .login-split .split-overlay {
					opacity: <?php echo esc_attr( floatval( $login_modern_overlay_opacity ) / 100 ); ?>;
				}
			}
			<?php
		}
		...
		// Form.
		if ( ! reign_is_truthy( $admin_login_form_transparency ) ) {
			?>
		...
		if ( ! empty( $login_form_background_color ) && ! reign_is_truthy( $admin_login_form_transparency ) ) {
			?>
			body.login #login {
		...
		if ( ! empty( $login_form_background_image ) && ! reign_is_truthy( $admin_login_form_transparency ) ) {
			?>
			body.login #login {
		...
		if ( ! empty( $login_form_shadow && $login_form_shadow_opacity ) ) {
			?>
			body.login #login {
				box-shadow: 0 0 <?php echo esc_attr( $login_form_shadow ); ?> rgb(0, 0, 0, <?php echo esc_attr( floatval( $login_form_shadow_opacity ) / 100 ); ?> ) !important;
			}
			<?php
		}
		...
		if ( ! empty( $login_button_shadow ) && $login_button_shadow_opacity ) {
			?>
			body.login #login .button-primary {
				box-shadow: 0 0 <?php echo esc_attr( $login_button_shadow ); ?> rgb(0, 0, 0, <?php echo esc_attr( floatval( $login_button_shadow_opacity ) / 100 ); ?> ) !important;
			}
			<?php
		}
