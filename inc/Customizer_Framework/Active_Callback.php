<?php
/**
 * Reign\Customizer_Framework\Active_Callback — array→closure adapter.
 *
 * @package Reign */
	...
	public static function compile( array $conditions ): callable {
		return static function () use ( $conditions ) {
			foreach ( $conditions as $cond ) {
				...
				// Default to empty string so a never-saved select / radio
				// setting evaluates correctly against operator '!=' (which
				// otherwise compared bool false to a string slug and hid
				// dependent controls until the user saved the parent once).
				...
				switch ( $operator ) {
					...
					case 'in':
						...
						break;
					case 'contains':
						// Membership test, with BOTH operands guarded so neither an
						// array-valued setting (repeater/sortable/multicheck) nor an
						// array condition value is ever cast to string. The string
						// cast emitted an "Array to string conversion" warning which,
						// with display_errors on, prints into the customizer responses
						// and breaks the customizer screen.
						if ( is_array( $actual ) ) {
							// Array setting must contain the expected value.
							if ( ! in_array( $expected, $actual, false ) ) {
								return false;
							}
						} elseif ( is_array( $expected ) ) {
							// Setting value must be one of the expected values.
							if ( ! in_array( $actual, $expected, false ) ) {
								return false;
							}
						} elseif ( false === strpos( (string) $actual, (string) $expected ) ) {
							return false;
						}
						break;
				}
