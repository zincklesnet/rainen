<?php
/**
 * Reign\Customizer_Framework\Controls\Repeater
 *
	...
	public function render_content() {
		...
		<fieldset class="buddyx-repeater" data-setting="<?php echo esc_attr( $this->setting ? $this->setting->id : '' ); ?>">
			...
			<button type="button" class="button buddyx-repeater-add">
				<?php
				// row_label may be the Kirki array shape { type, value, field }; use its scalar label for the button.
				$row_label = is_array( $this->row_label )
					? ( isset( $this->row_label['value'] ) ? $this->row_label['value'] : __( 'Item', 'reign' ) )
					: $this->row_label;
				/* translators: %s: row label, e.g. "Item" */
				printf( esc_html__( 'Add %s', 'reign' ), esc_html( $row_label ) );
				?>
			</button>
