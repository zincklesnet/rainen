...
function reign_bypass_conditional_assets() {
	...
	if ( is_admin() && function_exists( 'get_current_screen' ) ) {
		...
		if ( $screen ) {
			...
			if ( $screen->base === 'post' && in_array( $screen->post_type, get_post_types() ) ) {
				return true;
			}
		}
	}
...
function reign_get_current_page_widgets() {
	...
	foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
		...
		if ( empty( $widgets ) || $sidebar_id === 'wp_inactive_widgets' || ! is_array( $widgets ) ) {
			continue;
		}
		
		// For now, include all active widgets from all sidebars
		// This ensures widgets in footer, sidebar, etc. are all detected
	...
	foreach ( $elements as $element ) {
		...
		if ( isset( $element['elType'] ) && $element['elType'] === 'widget' ) {
			if ( isset( $element['widgetType'] ) ) {
				...
				if ( $widget_type === 'wp-widget' || $widget_type === 'wordpress' ) {
...
function reign_debug_current_page_widgets() {
	...
	if ( empty( $current_widgets ) ) {
		...
		foreach ( $current_widgets as $widget ) {
			...
			$plugin_text = $plugin ? " → <strong style=\"color: {$color}\">{$plugin}</strong>" : ' → <em>no assets</em>';
			echo '<li style="margin: 3px 0;">' . esc_html( $widget ) . $plugin_text . '</li>';
...
function reign_should_load_learndash_assets() {
	...
	if ( 
		( is_search() && isset( $_GET['post_type'] ) && $_GET['post_type'] === 'sfwd-courses' ) ||
...
function reign_should_load_tutorlms_assets() {
	...
	if ( isset($_GET['view']) && $_GET['view'] === 'instructor' || isset($_GET['view']) && $_GET['view'] === 'student' ) {
	    $username = get_query_var('tutor_profile_username') ?: (isset($_GET['tutor_profile_username']) ? sanitize_text_field($_GET['tutor_profile_username']) : '');
	    if ( ! empty( $username ) ) {
	        return true;
	    }
	}
...
function reign_should_load_wp_job_manager_assets() {
	...
	if (
	    is_singular( ['job_listing', 'resume'] ) ||
	    is_post_type_archive( ['job_listing', 'resume'] ) ||
	    is_tax( get_object_taxonomies( ['job_listing', 'resume'] ) )
	) {
	    return true;
	}
	...
	foreach ( $current_page_widgets as $widget_id ) {
		if ( strpos( $widget_id, 'job_manager' ) !== false || 
			 strpos( $widget_id, 'wp_job' ) !== false ||
			 strpos( $widget_id, 'resume_manager' ) !== false ||
			 strpos( $widget_id, 'job' ) !== false && strpos( $widget_id, 'manager' ) !== false ||
			 strpos( $widget_id, 'job' ) !== false && strpos( $widget_id, 'listing' ) !== false ||
			 strpos( $widget_id, 'resume' ) !== false ) {
			return true;
		}
	}
...
function reign_should_load_bp_better_messages_assets() {
	...
	if ( function_exists( 'Better_Messages' ) ) {
		...
		if ( $bm_instance && isset( $bm_instance->settings ) && is_array( $bm_instance->settings ) ) {
			...
			if ( isset( $settings['miniWidgetsEnable'] ) && $settings['miniWidgetsEnable'] == '1' ) {
				return true;
			}
		}
	}
...
function reign_should_load_woocommerce_assets( $asset_type = '' ) {
	...
	if ( class_exists( 'Reign_Dokan_Addon' ) && function_exists( 'bp_current_component' ) ) {
		...
		// Store tab
		if ( $current_component === 'store' ) {
			return true;
		}

		// Wishlists tab
		if ( $current_action === 'whislists' ) {
			return true;
		}

		// Favorite tab
		if ( $current_action === 'favorite' ) {
			return true;
		}
	}
	...
	if ( class_exists( 'Reign_Wcvendors_Addon' ) && function_exists( 'bp_current_component' ) ) {
		...
		if ( $current_component === 'favorite' || $current_component === 'store' ) {
			return true;
		}
	}
...
function reign_should_load_fluentcart_assets() {
    ...
    // Load assets whenever FluentCart is active
    $should_load = true;

    return apply_filters( 'reign_load_fluentcart_assets', $should_load );
}
...
function reign_should_load_product_roadmap_assets() {
    ...
    // Load assets whenever PrductRoadmap is active
    $should_load = true;

    return apply_filters( 'reign_load_product_roadmap_assets', $should_load );
}
