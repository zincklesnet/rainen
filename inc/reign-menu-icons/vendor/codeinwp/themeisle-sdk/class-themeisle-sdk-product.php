<?php
/**
 * The product model class for Reign SDK
 *
 * @package     ReignSDK
 * @subpackage  Product
...
if ( ! class_exists( 'Reign_SDK_Product' ) ) :
	/**
	 * Product model for Reign SDK.
	 */
	class Reign_ct {
		/**
		...
		public function setup_from_fileheaders() {
			...
			if ( $this->is_external_author() ) {
				$this->store_url  = 'https://reigntheme.com';
				$this->store_name = 'Reign';
			}
		...
		public function is_logger_active() {
			...
			if ( ! $this->is_wordpress_available() ) {
				...
				if ( ! empty( $pro_slug ) ) {
					...
					$all_products = Reign_r::get_products();
