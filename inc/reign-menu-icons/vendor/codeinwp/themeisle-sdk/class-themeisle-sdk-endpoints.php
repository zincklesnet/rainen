		...
		function rest_register() {
			register_rest_route(
				...
				array(
					...
					'permission_callback' => array( $this, 'checksum_permission_check' ),
				)
			);
		}

		/**
		 * Permission check for the checksum endpoint.
		 *
		 * @param WP_REST_Request $request The current request.
		 * @return bool|WP_Error True if the request has access, WP_Error otherwise.
		 */
		public function checksum_permission_check( $request ) {
			if ( current_user_can( 'manage_options' ) ) {
				return true;
			}
			return new WP_Error(
				'rest_forbidden',
				__( 'You do not have permissions to access this endpoint.', 'reign' ),
				array( 'status' => 403 )
			);
		}
