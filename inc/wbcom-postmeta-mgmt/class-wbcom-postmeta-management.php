<?php
/**
 * Wbcom Postmeta Management class.
 *
 * @package Reign
 */
...
if ( ! class_exists( 'Wbcom_Postmeta_Management' ) ) :
	...
	class Wbcom_Postmeta_Management {
		...
		public function includes() {
			include_once REIGN_INC_DIR . 'wbcom-postmeta-mgmt/class-wbcom-render-postmeta-fields.php';
			include_once REIGN_INC_DIR . 'wbcom-postmeta-mgmt/sections/class-wbcom-postmeta-mgmt-layout-section.php';
		}
		...
		public function enqueue_reign_metabox_style_n_script() {
			...
			wp_register_style(
				'wbcom-postmeta-mgmt',
				$url_prefix . '/assets/wbcom-postmeta-mgmt.css',
				array(),
				REIGN_THEME_VERSION,
				'all'
			);
			...
			wp_register_style(
				'select2.min.css',
				$url_prefix . '/assets/select2.min.css',
				array(),
				REIGN_THEME_VERSION,
				'all'
			);
			...
			wp_register_script(
				'select2.min.js',
				$url_prefix . '/assets/select2.min.js',
				array( 'jquery' ),
				REIGN_THEME_VERSION,
				true
			);
			...
			wp_register_script(
				'wbcom-postmeta-mgmt',
				$url_prefix . '/assets/wbcom-postmeta-mgmt.js',
				array( 'jquery', 'select2.min.js' ),
				REIGN_THEME_VERSION,
				true
			);
			...
			wp_register_script(
				...
				$url_prefix . '/assets/wp-color-picker-alpha.js',
		...
		public function render_reign_add_meta_box( $post ) {
			...
			if ( ! empty( $vertical_tabs ) && is_array( $vertical_tabs ) ) {
				...
				foreach ( $vertical_tabs as $key => $value ) {
					printf(
						'<div class="wbcom-metabox-content %1$s"%2$s>',
						esc_attr( $key ),
		...
		public function reign_add_meta_box() {
			global $post;
			if ( isset( $post ) && get_option( 'page_for_posts' ) == $post->ID && empty( $post->post_content ) ) {
				return;
			}
			$args = array(
				'public'   => true,
		...
		public function render_reign_add_post_format_meta_box( $post ) {
			...
			<div class="reign_post_format-settings">
				...
				<div class="reign_gallery_format_setting">
					...
					<ul class="reign_images_gallery images_gallery">
						...
						if ( ! empty( $post_image_gallery ) ) {
							...
							foreach ( $post_image_gallery as $image_id ) {
								...
								if ( $image_id > 0 ) {
									printf(
										'<li class="image" data-attachment_id="%1$s">
											...
											<div class="actions">
												<a href="#" id="%1$s" class="delete" title="%3$s"><i class="dashicons dashicons-no"></i></a>
											</div>
										</li>',
										esc_attr( $image_id ),
										wp_kses_post( wp_get_attachment_image( $image_id, 'thumbnail' ) ),
										esc_attr__( 'Delete image', 'reign' )
									);
					...
					<input type="hidden" id="reign_image_gallery" name="reign_image_gallery" value="<?php echo esc_attr( rtrim( $image_gallery, ',' ) ); ?>" />
					</div>
					<div class="clearfix reign_image_gallery_description">
						<p class="add_reign_images hide-if-no-js">
							<a class="components-button is-primary" href="#"><?php esc_html_e( 'Add Image Gallery', 'reign' ); ?></a>
						</p>
					</div>
				</div>
					...
					$( document ).ready( function () {
						...
						$('.add_reign_images').on( 'click', 'a', function( event ) {
							...
							image_gallery_frame = wp.media.frames.downloadable_file = wp.media({
								...
								title: '<?php echo esc_js( __( 'Add Image Gallery', 'reign' ) ); ?>',
								button: {
									text: '<?php echo esc_js( __( 'Add to gallery', 'reign' ) ); ?>',
								},
								multiple: true
							});
									...
									if ( attachment.id ) {
										...
										$images_gallery.append('\
											<li class="image" data-attachment_id="' + attachment.id + '">\
																							...
																							<div class="actions">\
																								<a href="#" class="delete" title="<?php echo esc_js( __( 'Delete image', 'reign' ) ); ?>"><i class="dashicons dashicons-no"></i></a>\
																							</div>\
											</li>');
									}
								} );
						...
						$('.reign_upload_media').on( 'click',  function( event ) {
							...
							image_gallery_frame = wp.media.frames.downloadable_file = wp.media({
								/* Set the title of the modal.  */
								title: '<?php echo esc_js( __( 'Add Media', 'reign' ) ); ?>',
								button: {
									text: '<?php echo esc_js( __( 'Add to Library', 'reign' ) ); ?>',
								},
								multiple: true
							});
							/* When an image is selected, run a callback.  */
		...
		public function reign_save_post_meta( $post_id ) {
			...
			$reign_post_types_support = Reign_Customizer_Post_Types_Fields::instance();
				...
				if ( 'post' === $post_type || 'page' === $post_type || in_array( $post_type, (array) $post_type_group, true ) ) {
					...
					foreach ( $vertical_tabs as $key => $value ) {
						if ( isset( $_POST[ $key ] ) && is_array( $_POST[ $key ] ) && ! empty( $_POST[ $key ] ) ) {
							$raw_tab = wp_unslash( $_POST[ $key ] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- each scalar value is sanitized with sanitize_text_field() in the loop below.
					...
					if ( isset( $_POST['subheader'] ) && is_array( $_POST['subheader'] ) ) {
						$raw_subheader = wp_unslash( $_POST['subheader'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- each scalar value is sanitized with sanitize_text_field() in the loop below.
