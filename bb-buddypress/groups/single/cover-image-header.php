...
$profile_group_layout_style   = bb_platform_group_header_style();
$override_group_header_layout = $wbtm_reign_settings['reign_buddyextender']['override_group_header_layout'] ?? '';

if ( function_exists( 'bb_platform_pro' ) && 'on' !== $override_group_header_layout ) {
	if ( 'centered' === $profile_group_layout_style ) {
		$group_header_class = 'wbtm-cover-header-type-4';
	} elseif ( 'left' === $profile_group_layout_style ) {
		$group_header_class = 'wbtm-cover-header-type-3';
	}
}
	...
	<div id="header-cover-image" class="<?php echo esc_attr( 'cover-' . $group_cover_height . ' width-' . $group_cover_width . $has_cover_image_position . $has_cover_image . $has_default_cover ); ?>">
		...
		if ( bp_group_use_cover_image_header() ) {
			...
			<?php if ( bp_is_item_admin() ) { ?>
				<a href="<?php echo esc_url( $group_cover_link ); ?>" class="link-change-cover-image bp-tooltip" data-bp-tooltip-pos="right" data-bp-tooltip="<?php esc_attr_e( 'Change Cover Photo', 'reign' ); ?>" aria-label="<?php esc_attr_e( 'Change Cover Photo', 'reign' ); ?>">
		...
		<div class="wbtm-member-info-section"><!-- custom wrapper for main content :: start -->
			...
			<?php if ( ! bp_disable_group_avatar_uploads() ) : ?>
				<div id="item-header-avatar">
					<?php if ( bp_is_item_admin() ) { ?>
						<a href="<?php echo esc_url( $group_avatar ); ?>" class="link-change-profile-image bp-tooltip" data-bp-tooltip-pos="down" data-bp-tooltip="<?php esc_attr_e( 'Change Group Photo', 'reign' ); ?>" aria-label="<?php esc_attr_e( 'Change Group Photo', 'reign' ); ?>">
			...
			<div class="modal-wrapper">
				<div class="modal-container">
					<header class="bb-model-header">
						...
						<a class="bb-close-action-popup bb-model-close-button" href="#" aria-label="<?php esc_attr_e( 'Close', 'reign' ); ?>">
