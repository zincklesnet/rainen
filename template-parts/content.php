...
<article id="post-<?php the_ID(); ?>" <?php post_class( $blog_list_layout ); ?>>
	...
	<?php
	if ( ! is_singular( 'sfwd-courses' ) && ! is_singular( 'groups' ) ) {
		// Check for custom featured image position on singular posts.
		$fi_custom_output = false;
		if ( is_singular() && function_exists( 'reign_render_featured_image' ) ) {
			$fi_custom_output = reign_render_featured_image();
		}

		if ( false !== $fi_custom_output ) {
			// Custom position layout (behind, beside, or hidden).
			echo $fi_custom_output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
